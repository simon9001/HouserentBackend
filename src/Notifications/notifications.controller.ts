...
// Define a custom context type that includes user
interface AuthContext extends Context {
    user?: TokenPayload;
}

// Helper function to get user from context (handles both methods)
const getUserFromContext = (c: AuthContext): TokenPayload | undefined => {
    // Try both methods: c.get('user') and c.user
    const userFromGet = c.get('user') as TokenPayload | undefined;
...
const isValidNotificationType = (type: string): type is 'BOOKING' | 'PAYMENT' | 'REVIEW' | 'SYSTEM' | 'ALERT' => {
...
export const markAsRead = async (c: AuthContext) => {
    try {
...
export const markAllAsRead = async (c: AuthContext) => {
    try {
...
export const getUnreadCount = async (c: AuthContext) => {
    try {
...
export const sendBroadcast = async (c: AuthContext) => {
    try {
...
export const sendToClients = async (c: AuthContext) => {
    try {
...
export const createNotification = async (c: AuthContext) => {
    try {
...
export const getNotificationById = async (c: AuthContext) => {
    try {
...
export const debugContext = async (c: AuthContext) => {
