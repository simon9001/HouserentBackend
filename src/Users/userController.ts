...
import { usersService, CreateUserInput, UpdateUserInput } from '../Users/userService.js';
...
// Update agent status
export const updateAgentStatus = async (c: Context) => {
    try {
        const userId = c.req.param('userId');
        const body = await c.req.json();
...
// Verify user email
export const verifyUserEmail = async (c: Context) => {
    try {
        const userId = c.req.param('userId');
...
// Get user statistics
export const getUserStatistics = async (c: Context) => {
    try {
...
export const getUsersByRole = async (c: Context) => {
    try {
        const role = c.req.param('role') as 'TENANT' | 'AGENT' | 'ADMIN';
        const page = parseInt(c.req.query('page') || '1');
        const limit = parseInt(c.req.query('limit') || '20');
