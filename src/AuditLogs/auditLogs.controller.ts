...
import { auditLogsService, CreateAuditLogInput } from './auditLogs.service.js';
...
export const createAuditLog = async (c: Context) => {
    try {
        ...
        const logData: CreateAuditLogInput = {
            userId: body.userId,
            action: body.action,
            entity: body.entity,
...
export const getAuditLogById = async (c: Context) => {
    try {
...
export const getAuditLogsByUserId = async (c: Context) => {
    try {
...
export const getAuditLogsByEntity = async (c: Context) => {
    try {
...
export const searchAuditLogs = async (c: Context) => {
    try {
        ...
        const { logs, total } = await auditLogsService.searchLogs(
            searchTerm.trim(),
            userId,
            entity,
            startDate,
...
export const getAuditStatistics = async (c: Context) => {
    try {
        const startDate = c.req.query('startDate') ? new Date(c.req.query('startDate')!) : undefined;
        const endDate = c.req.query('endDate') ? new Date(c.req.query('endDate')!) : undefined;
...
export const cleanOldAuditLogs = async (c: Context) => {
    try {
        const body = await c.req.json();
