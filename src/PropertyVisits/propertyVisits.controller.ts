...
import { propertyVisitsService, CreateVisitInput, UpdateVisitInput } from './propertyVisits.service.js';
...
export const getVisitsByPropertyId = async (c: Context) => {
    try {
...
export const getVisitsByUserId = async (c: Context) => {
    try {
        ...
        const visits = await propertyVisitsService.getVisitsByUserId(userId, role as 'tenant' | 'agent');
...
export const updateVisit = async (c: Context) => {
    try {
        ...
        if (body.visitPurpose !== undefined) updateData.visitPurpose = body.visitPurpose;
        if (body.tenantNotes !== undefined) updateData.tenantNotes = body.tenantNotes;
        if (body.agentNotes !== undefined) updateData.agentNotes = body.agentNotes;
        if (body.status !== undefined) updateData.status = body.status;
        if (body.checkInTime !== undefined) updateData.checkInTime = new Date(body.checkInTime);
        if (body.checkOutTime !== undefined) updateData.checkOutTime = new Date(body.checkOutTime);
...
// Cancel visit
export const cancelVisit = async (c: Context) => {
    try {
        const visitId = c.req.param('visitId');
        const body = await c.req.json();
...
// Check in
export const checkIn = async (c: Context) => {
    try {
        const visitId = c.req.param('visitId');
...
// Get upcoming visits
export const getUpcomingVisits = async (c: Context) => {
    try {
...
export const getVisitStatistics = async (c: Context) => {
    try {
