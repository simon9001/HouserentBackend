// propertyMedia.controller.ts (FIXED VERSION)
import { Context } from 'hono';
import { propertyMediaService } from './propertyMedia.service.js';
import { ValidationUtils } from '../utils/validators.js';
import { CloudinaryService } from '../services/cloudinary.service.js';
import type { CreateMediaInput } from './propertyMedia.service.js';

// ==================== FILE UPLOAD ENDPOINTS ====================

// Create media with file upload (backend handles Cloudinary)
export const createMediaWithUpload = async (c: Context) => {
  try {
    ...
    const mediaTypeValue = body.mediaType as string;
    ...
    const cloudinaryResult = await CloudinaryService.uploadFile(buffer, {
      ...
      resourceType: mediaType.toLowerCase() as 'image' | 'video' | 'raw',
...
export const createBulkMediaWithUpload = async (c: Context) => {
  try {
    const body = await c.req.parseBody();
    ...
    const propertyId = body.propertyId as string;
    const isPrimaryIndex = parseInt(body.primaryIndex as string) || 0;
    ...
    const mediaRecords: CreateMediaInput[] = cloudinaryResults.map((result, index) => ({
      propertyId,
...
export const getUploadSignature = async (c: Context) => {
  try {
...
export const deleteMediaWithCloudinary = async (c: Context) => {
  try {
    ...
    if (media.cloudinary_public_id) {
      ...
      const resourceType = (media.media_type || '').toLowerCase() as 'image' | 'video' | 'raw';
...
export const createBulkMedia = async (c: Context) => {
  try {
    ...
    const validMediaTypes = ['IMAGE', 'VIDEO', 'DOCUMENT'];
    const mediaRecords: CreateMediaInput[] = [];
