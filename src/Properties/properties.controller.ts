...
import { propertiesService, CreatePropertyInput, UpdatePropertyInput, PropertyFilter } from './properties.service.js';
...
export const createProperty = async (c: Context) => {
    try {
        ...
        const missingFields = requiredFields.filter(field => !transformedBody[field as keyof typeof transformedBody]);
        ...
        const property = await propertiesService.createProperty(transformedBody as CreatePropertyInput);
...
export const getPropertiesByOwner = async (c: Context) => {
    try {
...
export const updateProperty = async (c: Context) => {
    try {
        ...
        Object.keys(updateData).forEach(key => {
            if (updateData[key as keyof UpdatePropertyInput] === undefined) {
        ...
        if (updateData.property_type) {
            ...
            updateData.property_type = updateData.property_type.toUpperCase() as any;
        }
...
export const getPropertyStatistics = async (c: Context) => {
    try {
...
export const verifyProperty = async (c: Context) => {
    try {
        const propertyId = c.req.param('propertyId');
...
export const boostProperty = async (c: Context) => {
    try {
        const propertyId = c.req.param('propertyId');
        const body = await c.req.json();
...
export const getFeaturedProperties = async (c: Context) => {
    try {
        const page = parseInt(c.req.query('page') || '1');
...
export const getSimilarProperties = async (c: Context) => {
    try {
        const propertyId = c.req.param('propertyId');
        ...
        const similarProperties = result.properties.filter((p: any) => p.property_id !== propertyId);
