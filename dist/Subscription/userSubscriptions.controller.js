...
export const createUserSubscription = async (c) => {
    try {
...
export const getUserSubscriptionById = async (c) => {
    try {
...
export const getMyActiveSubscription = async (c) => {
    try {
        const userId = c.get('userId'); // From auth middleware
...
export const getMySubscriptions = async (c) => {
    try {
        const userId = c.get('userId'); // From auth middleware
...
// Update subscription
export const updateUserSubscription = async (c) => {
    try {
        const subscriptionId = c.req.param('subscriptionId');
        const body = await c.req.json();
        const userId = c.get('userId'); // From auth middleware
...
// Cancel subscription
export const cancelUserSubscription = async (c) => {
    try {
        const subscriptionId = c.req.param('subscriptionId');
        const body = await c.req.json();
        const userId = c.get('userId'); // From auth middleware
        ...
        const cancelledSubscription = await userSubscriptionsService.cancelSubscription(subscriptionId, cancelAtPeriodEnd);
...
// Renew subscription
export const renewUserSubscription = async (c) => {
    try {
        const subscriptionId = c.req.param('subscriptionId');
        const body = await c.req.json();
        const userId = c.get('userId'); // From auth middleware
        ...
        const renewedSubscription = await userSubscriptionsService.renewSubscription(subscriptionId, body.paymentId);
        return c.json({
            success: true,
...
// Check usage limit
export const checkUsageLimit = async (c) => {
    try {
        const userId = c.get('userId'); // From auth middleware
...
// Record usage
export const recordUsage = async (c) => {
    try {
        const userId = c.get('userId'); // From auth middleware
        const body = await c.req.json();
        // Validate required fields
        ...
        await userSubscriptionsService.recordUsage(userId, body.feature, body.resourceId, body.action || 'CREATE', body.count || 1, body.override || false, body.overrideReason, body.ipAddress || c.req.header('x-forwarded-for') || c.req.header('x-real-ip'), body.userAgent || c.req.header('user-agent'), body.metadata);
        return c.json({
            success: true,
...
export const getUsageStatistics = async (c) => {
    try {
        const userId = c.get('userId'); // From auth middleware
...
//# sourceMappingURL=userSubscriptions.controller.js.map