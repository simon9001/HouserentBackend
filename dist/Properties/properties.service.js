...
export class PropertiesService {
    ...
    mapDBToPropertyWithRelations(data) {
        const property = this.mapDBToProperty(data);
        // Add owner info
        if (data.Users) {
            property.owner = {
        ...
        if (data.property_media && Array.isArray(data.property_media)) {
            property.media = data.property_media.map((m) => ({
        ...
        if (data.nearby_places && Array.isArray(data.nearby_places)) {
            property.nearby_places = data.nearby_places.map((p) => ({
        ...
        if (data.property_amenities && Array.isArray(data.property_amenities)) {
            property.amenities = data.property_amenities.map((a) => a.amenity_name);
        }
        return property;
    }
    ...
    async getPropertiesByOwner(ownerId, page = 1, limit = 20) {
        ...
        `, { count: 'exact' })
            ...
            return {
                properties: (data || []).map(p => this.mapDBToProperty(p)),
                total: count || 0,
                page,
    ...
    async getAllProperties(page = 1, limit = 20, filters = {}) {
        ...
        `, { count: 'exact' });
            ...
            if (filters.search_term) {
                query = query.or(`title.ilike.%${filters.search_term}%,` +
            ...
            const properties = (data || []).map(item => {
                const property = this.mapDBToProperty(item);
                // Add owner info
                if (item.Users) {
                    property.owner = {
                ...
                if (item.property_media && item.property_media.length > 0) {
                    const primaryMedia = item.property_media.find((m) => m.is_primary) || item.property_media[0];
    ...
    async updateProperty(propertyId, data) {
        try {
            ...
            if (data.rent_amount !== undefined)
                updateData.rent_amount = data.rent_amount;
            ...
            if (data.is_boosted !== undefined)
                updateData.is_boosted = data.is_boosted;
            if (data.boost_expiry !== undefined)
                updateData.boost_expiry = data.boost_expiry;
    ...
    async boostProperty(propertyId, days = 7) {
