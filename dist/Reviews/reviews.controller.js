...
// Get review by ID
export const getReviewById = async (c) => {
    try {
...
export const getReviewsByAgentId = async (c) => {
    try {
...
export const getReviewsByPropertyId = async (c) => {
    try {
...
// Update review
export const updateReview = async (c) => {
    try {
        const reviewId = c.req.param('reviewId');
        const body = await c.req.json();
        if (!ValidationUtils.isValidUUID(reviewId)) {
...
export const getAgentRatingSummary = async (c) => {
    try {
        const agentId = c.req.param('agentId');
        if (!ValidationUtils.isValidUUID(agentId)) {
            return c.json({
...
export const getPropertyRatingSummary = async (c) => {
    try {
        const propertyId = c.req.param('propertyId');
        if (!ValidationUtils.isValidUUID(propertyId)) {
            return c.json({
