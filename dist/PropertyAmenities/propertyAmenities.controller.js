...
// Create new amenity
export const createAmenity = async (c) => {
    try {
...
// Get amenity by ID
export const getAmenityById = async (c) => {
    try {
...
export const getAmenitiesByPropertyId = async (c) => {
    try {
...
// Update amenity
export const updateAmenity = async (c) => {
    try {
        const amenityId = c.req.param('amenityId');
        const body = await c.req.json();
        if (!ValidationUtils.isValidUUID(amenityId)) {
...
// Delete amenity
export const deleteAmenity = async (c) => {
    try {
        const amenityId = c.req.param('amenityId');
        if (!ValidationUtils.isValidUUID(amenityId)) {
            return c.json({
...
export const createBulkAmenities = async (c) => {
    try {
        const body = await c.req.json();
        // Validate required fields
        ...
        const validAmenities = body.amenities
            .filter((amenity) => typeof amenity === 'string' && amenity.trim().length > 0)
...
export const deleteAmenitiesByPropertyId = async (c) => {
    try {
        const propertyId = c.req.param('propertyId');
        if (!ValidationUtils.isValidUUID(propertyId)) {
            return c.json({
...
export const getCommonAmenities = async (c) => {
    try {
...
// Search amenities
export const searchAmenities = async (c) => {
    try {
        ...
        const amenities = await propertyAmenitiesService.searchAmenities(searchTerm.trim(), propertyId);
        return c.json({
            success: true,
            data: amenities
        });
...
export const getAmenitiesStatistics = async (c) => {
    try {
        const propertyId = c.req.query('propertyId');
        if (propertyId && !ValidationUtils.isValidUUID(propertyId)) {
            return c.json({
...
//# sourceMappingURL=propertyAmenities.controller.js.map